...
async def execute():
    ...
    ASSET_TX_URL = {
        ...
        'BSV':['https://explorer.bitquery.io/bitcoinsv/tx/'],
    ...
    for file in files:
        ...
        work_book = load_workbook(f'{INPUT_PATH}{file}', data_only=True)
        ...
        for worksheet_idx in range(len(work_book.worksheets)):
            ...
            output = open(f'{OUTPUT_PATH}{os.path.splitext(file)[0]}_sheet_{worksheet_idx}.csv', 'w', newline='')
            writer = csv.writer(output)
            writer.writerow(['Asset', 'Transaction ID', 'Blockchain URL', 'Is Verified'])
            ...
            async def verify(session, prefixes, asset, tx_id):
                is_verified = False
                for prefix in prefixes:
                    ...
                    try:
                        ...
                        print(f'Error processing {tx_url}')
                        # await invalidate(row_idx, tx_url_idx)
                        # writer.writerow([asset, tx_id, tx_url, 'No'])
                if is_verified:
                    writer.writerow([asset, tx_id, tx_url, 'Yes'])
                else:
                    writer.writerow([asset, tx_id, tx_url, 'No'])
            ...
            for row_idx in range(2, row_count + 1):
                ...
                if tx_id_idx is not None and tx_url_idx is not None:
                    ...
                    task = asyncio.ensure_future(verify(session, prefixes, asset.upper(), tx_id))
